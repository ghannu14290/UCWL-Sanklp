...
//  DDMenuController
//
// Created by Sanjeev Bajpai on 28/05/15.
//  Copyright (c) 2015 Codecube. All rights reserved.

#import "DDMenuController.h"

#define kMenuFullWidth self.view.bounds.size.width
#define kMenuDisplayedWidth (self.view.bounds.size.width - 55)
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _menuFlags.showingRootView = YES;
...
- (void)resetNavButtons {
    ...
    if (_menuFlags.canShowLeft) {
        ...
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 10,30, 40)];
        [btn setImage:[UIImage imageNamed:@"menu"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(showLeft:) forControlEvents:UIControlEventAllTouchEvents];
        
//        UIView *view = [[UIView alloc] initWithFrame:btn.frame];
//        view.bounds = CGRectOffset(view.bounds,20,0);
//        [view addSubview:btn];
        
        UIBarButtonItem *button = [[barButtonItemClass alloc] initWithCustomView:btn];
    ...
    if (_menuFlags.canShowRight) {
        ...
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 75, 44)];
        ...
        UIView *view = [[UIView alloc] initWithFrame:btn.frame];
        view.bounds = CGRectOffset(view.bounds, -15, 0);
        [view addSubview:btn];
        
        UIBarButtonItem *button = [[barButtonItemClass alloc] initWithCustomView:view];
...
- (void)setRootViewController:(UIViewController *)rootViewController {
    ...
    _menuFlags.canShowLeft = (_root!=nil);
...
- (void)setRootController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    if (_menuFlags.showingLeftView || _menuFlags.showingRightView) {
        ...
        frame.origin.x = rootRef.view.bounds.size.width * (_menuFlags.showingLeftView ? 1 : -1);
        //rootRef.view.bounds.size.width * _menuFlags.showingLeftView ? 1 : -1;
